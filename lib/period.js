...
// Chap nhan ca SO lan CHUOI so (cot BIGINT tu Postgres tra ve dang chuoi);
// gia tri khong hop le thi dung thoi diem hien tai thay vi tra ve NaN.
function toMs(atTime) {
  const n = Number(atTime);
...
function getWeekStart(atTime = Date.now()) {
  const now = new Date(toMs(atTime));
  const utc = now.getTime() + now.getTimezoneOffset() * 60000;
  const gmt7 = new Date(utc + 7 * 3600000);
