const db = require('../db');
const { ipMatchClause } = require('./ip');
...
async function evaluateUserRisk(userId, { ip = '', fingerprint = '', userAgent = '' } = {}) {
  ...
  if (ip) {
    const m = ipMatchClause(1, ip);
    const r = await db.get(`SELECT COUNT(*)::int as c FROM ip_user_map WHERE ${m.clause} AND user_id!=$${m.params.length + 1}`, [...m.params, userId]);
