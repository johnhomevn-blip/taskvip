...
async function distributeWeeklyTaskRewards() {
  ...
  // VA LOI DA SUA (2026-09): truoc day ham nay LUON trao thuong bat ke setting
  // 'ranking_enabled' (dung de an/hien trang /ranking) dang bat hay tat - tuc
  // la admin tat hien thi BXH tren giao dien nhung tien VAN tu dong duoc cong
  // cho Top 1/2/3 moi tuan o phia sau, khong ai biet. Gio kiem tra setting nay
  // truoc: neu dang TAT, khong trao thuong cho tuan do, nhung VAN danh dau
  // "da xu ly" (cap nhat last_weekly_reward_week) de tranh truong hop sau nay
  // bat lai BXH thi bi tra thuong don (retroactive) cho nhung tuan da tat.
  const rankingEnabled = (await getSetting('ranking_enabled')) !== '0';
  if (!rankingEnabled) {
...
async function distributeMonthlyReferralRewards() {
  ...
  // Setting rieng cho BXH gioi thieu (doc lap voi 'referral_enabled' - cai do
  // tat ca he thong hoa hong gioi thieu; cai nay CHI tat rieng BANG XEP HANG +
  // thuong Top 1/2/3 hang thang, hoa hong % van chay binh thuong neu chi tat
  // muc nay). Tat thi khong trao thuong thang do, nhung van danh dau "da xu
  // ly" de tranh tra thuong don khi bat lai sau nay.
  const refRankingEnabled = (await getSetting('referral_ranking_enabled')) !== '0';
  if (!refRankingEnabled) {
